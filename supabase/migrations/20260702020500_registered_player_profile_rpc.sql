-- Create PokerTrack player profiles through a narrow security-definer RPC.
-- This avoids direct table writes from the Edge Function while still requiring
-- the caller to be an authenticated host.
...
begin
  ...
  if clean_username !~ '^[a-z0-9][a-z0-9_-]{2,31}$' then
